  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        showTransparent ? "bg-transparent py-4" : "bg-background/95 backdrop-blur-lg shadow-lg py-2"
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="relative">
              <img src={logoIcon} alt="Cross Angle Interior" className="h-12 md:h-14 w-auto" />
            </div>
            {/* Animated Brand Text */}
            <div className="font-serif text-xl md:text-2xl font-bold animate-logo-pulse-glow transition-all duration-500 group-hover:animate-logo-spin">
              ...
              <span
                className={`transition-colors duration-300 ${showTransparent ? "text-primary-foreground" : "text-foreground"}`}
              >
                Interior
              </span>
            </div>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span
                  className={`absolute -bottom-1 left-0 h-0.5 bg-primary transition-all duration-300 ${location.pathname === link.href ? "w-full" : "w-0 group-hover:w-full"}`}
                />
              </Link>
            ))}
          </div>
          ...
          <div className="bg-background/95 backdrop-blur-lg rounded-2xl p-6 border border-border shadow-xl">
            <div className="flex flex-col gap-4">
              ...
              <div className="pt-4 flex flex-col gap-3">
                <a href="tel:+917909041132" className="flex items-center gap-2 text-muted-foreground font-medium">
                  <Phone className="w-4 h-4" />
                  <span>+91 7909041132</span>
                </a>
