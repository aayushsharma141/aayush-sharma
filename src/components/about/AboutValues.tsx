...
const ValueCard = ({ icon: Icon, title, description, index }: ValueCardProps) => {
  ...
  return (
    ...
    >
      <div className="relative h-full p-8 rounded-3xl bg-card/50 backdrop-blur-md border border-border/50 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 overflow-hidden">
        {/* Spotlight gradient */}
        <motion.div
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
          style={{
            background: isHovered
              ? `radial-gradient(400px circle at ${(mouseX.get() + 0.5) * 100}% ${(mouseY.get() + 0.5) * 100}%, hsl(var(--primary) / 0.1), transparent 40%)`
              : "none",
          }}
        />
        ...
        {/* Icon */}
        <motion.div
          className="w-14 h-14 rounded-2xl bg-muted/50 group-hover:bg-primary flex items-center justify-center mb-6 transition-all duration-500"
          whileHover={{ scale: 1.1, rotate: 5 }}
        >
          <Icon className="w-7 h-7 text-muted-foreground group-hover:text-primary-foreground transition-colors duration-500" />
        </motion.div>
